...
for i=1:nc
wfull(i,:,:)=conj(imresize(squeeze(abs(wsmall(i,:,:))),[ny nx],'bilinear')...
    .*exp(j.*imresize(angle(squeeze(wsmall(i,:,:))),[ny nx],'nearest')));
cmap(i,:,:)=imresize(squeeze(abs(cmapsmall(i,:,:))),[ny nx],'bilinear')...
    .*exp(j.*imresize(squeeze(angle(cmapsmall(i,:,:))),[ny nx],'nearest'));
end
